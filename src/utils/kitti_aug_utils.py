...
def lidar_to_bird_view(x, y, factor=1):
	x_r = [cnf.boundary["minX"], cnf.boundary["maxX"]]
	y_r = [cnf.boundary["minY"], cnf.boundary["maxY"]]
	z_r = [cnf.boundary["minZ"], cnf.boundary["maxZ"]]
	res = cnf.DISCRETIZATION
	a = -(y + int(y_r[0])/res)
	b = x/res
...
def voxelNetAugScheme(lidar, labels, augData):
	...
	# Here Perturbation is not applied in default state
	# Start low = 0, to include Perturbation
	choice = np.random.randint(low=1, high=4)
