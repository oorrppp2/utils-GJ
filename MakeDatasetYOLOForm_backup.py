# from scipy import io
# from matplotlib import pyplot
# from mpl_toolkits.mplot3d import Axes3D
# from numpy import matlib
# import numpy as np
# import cv2
# import math
# import sys
#
# def flip_towards_viewer(normals, points):
#     mat = np.matlib.repmat(np.sqrt(np.sum(points*points, 1)), 3, 1)
#     points = points / mat
#     # print(points)
#     proj = np.sum(points * normals, 1)
#     flip = proj > 0
#     normals[flip, :] = -normals[flip, :]
#     return normals
#
# # np.set_printoptions(threshold=sys.maxsize)
#
#
# mat_file = io.loadmat('/home/user/SUNRGBD/SUNRGBDMeta.mat')
#
# path_tail = "/home/user/darknet/data/"
# # path_tail = "/home/user/"
#
# ID = {"bed":0, "book":1, "dishware":2, "box":3, "drawer":4, "chair":5,
#       "counter":6, "desk":7, "door":8, "lamp":9, "painting":10, "pillow":11,
#       "plant": 12, "shelf":13, "sofa":14, "table":15, "toilet":16, "tv":17}
# ID_cnt = np.zeros(18)
# # print("ID_cnt :", ID_cnt)
# classes = [
# "bed",
# "books",
# "book",
# "bottle",
# "bowl",
# "plate",
# "box",
# "cabinet",
# "drawer",
# "kitchen_cabinet",
# "file_cabinet",
# "chair",
# "kitchen_counter",
# "counter",
# "cup",
# "mug",
# "desk",
# "door",
# "lamp",
# "night_stand",
# "painting",
# "picture",
# "pillow",
# "plant",
# "plants",
# "flowers",
# "shelf",
# "bookshelf",
# "sofa_chair",
# "sofa",
# "table",
# "endtable",
# "coffee_table",
# "dining_table",
# "toilet",
# "tv"
# ]
#
# # print("size : ", len(classes))
#
# for range_index in range(4) :
#
#     data = mat_file.popitem()
#     if(data[0] == 'SUNRGBDMeta'):
#
#         # count_num = np.zeros(1146)
#         sample_size = np.arange(10335)
#         # sample_size = np.arange(10)
#
#         # print(count_num)
#         # classes = {}
#         classes_txt = ""
#         train_txt = ""
#         cls_s = path_tail+"3Dobj4.names"
#         train_s = path_tail+"3Dtrain4.txt"
#         file_num = 0
#
#
#         for ele in sample_size:
#             image_index = ele
#             if(data[1][0][image_index][1].size < 1):
#                 continue
#
#             # img_s = path_tail+"MyDataset_3D/" + str(ele) + ".tiff"
#             # img_s_3d = path_tail+"MyDataset_3D/" + str(ele) + ".tiff"
#             # txt_s = path_tail+"MyDataset_3D/" + str(ele) + ".txt"
#             # train_txt += "data/MyDataset_3D/" + str(ele) + ".tiff" + "\n"
#             img_s = path_tail + "MyDataset_2D/" + str(ele) + ".tiff"
#             # img_s_3d = path_tail+"MyDataset/" + str(ele) + ".tiff"
#             txt_s = path_tail + "MyDataset_2D/" + str(ele) + ".txt"
#             txt = ""
#
#             color_raw = cv2.imread(data[1][0][image_index][5][0], -1)
#             depth_raw = cv2.imread(data[1][0][image_index][4][0], -1)
#             row = len(color_raw)
#             col = len(color_raw[0])
#
#             for groundtruth3DBB in data[1][0][image_index][1]:
#                 for items in groundtruth3DBB:
#
#                     if(items[7].size < 1):
#                         continue
#
#                     """"check"""
#                     label = items[3][0]
#                     if label in classes:
#                         if label == "books":
#                             label = "book"
#                             pass
#                         elif label == "bottle" or label == "bowl" or label == "plate" or label == "cup" or label == "mug":
#                             label = "dishware"
#                             pass
#                         elif label == "cabinet" or label == "kitchen_cabinet" or label == "file_cabinet" or label == "night_stand":
#                             label = "drawer"
#                             pass
#                         elif label == "kitchen_counter":
#                             label = "counter"
#                             pass
#                         elif label == "picture":
#                             label = "painting"
#                             pass
#                         elif label == "plants" or label == "flowers":
#                             label = "plant"
#                             pass
#                         elif label == "bookshelf":
#                             label = "shelf"
#                             pass
#                         elif label == "sofa_chair":
#                             label = "sofa"
#                             pass
#                         elif label == "endtable" or label == "coffee_table" or label == "dining_table":
#                             label = "table"
#                             pass
#                         pass
#                     else:
#                         continue
#                         pass
#                     # ID_cnt[ID[label]] += 1
#
#                     # if items[3][0] not in classes:
#                     #     classes[items[3][0]] = index
#                     #     index += 1
#                     #     classes_txt += items[3][0] + "\n"
#                     # print("class : ",items[3][0]," / ",classes[items[3][0]])
#
#
#                     """ 3D bounding box """
#                     # inds = np.argsort(-abs(items[0][:, 0]))
#                     #
#                     # basis = items[0][inds, :]
#                     # coeffs = items[1][0, inds]
#                     #
#                     # # print("basis : ", basis)
#                     # # print("coeffs : ", coeffs)
#                     #
#                     # inds = np.argsort(-abs(basis[1:, 1]))
#                     #
#                     # centroid = items[2]
#                     # basis = flip_towards_viewer(basis, np.matlib.repmat(centroid, 3, 1))
#                     # coeffs = abs(coeffs)
#                     #
#                     # b1 = basis[0,0]
#                     # b2 = basis[0,1]
#                     # b3 = basis[1,0]
#                     # b4 = basis[1,1]
#                     #
#                     # c1 = coeffs[0]
#                     # c2 = coeffs[1]
#                     # c3 = coeffs[2]
#                     #
#                     # x = centroid[0,0]
#                     # y = centroid[0,1]
#                     # z = centroid[0,2]
#                     #
#                     # txt += str(classes[items[3][0]])
#                     # txt += " " + (str(b1)+"000000")[:8]
#                     # txt += " " + (str(b2)+"000000")[:8]
#                     # txt += " " + (str(b3)+"000000")[:8]
#                     # txt += " " + (str(b4)+"000000")[:8]
#                     #
#                     # txt += " " + (str(c1)+"000000")[:8]
#                     # txt += " " + (str(c2)+"000000")[:8]
#                     # txt += " " + (str(c3)+"000000")[:8]
#                     #
#                     # txt += " " + (str(x)+"000000")[:8]
#                     # txt += " " + (str(y)+"000000")[:8]
#                     # txt += " " + (str(z)+"000000")[:8] + "\n"
#
#
#                     """ 2D bounding box """
#                     gt2DBB = items[7][0]
#                     bx = (items[7][0][0]+items[7][0][2]/2)/col
#                     by = (items[7][0][1]+items[7][0][3]/2)/row
#                     bw = items[7][0][2]/col
#                     bh = items[7][0][3]/row
#
#                     txt += str(ID[label])
#                     txt += " " + (str(bx)+"000000")[:8]
#                     txt += " " + (str(by)+"000000")[:8]
#                     txt += " " + (str(bw)+"000000")[:8]
#                     txt += " " + (str(bh)+"000000")[:8] + "\n"
#                     # count_num[classes[items[3][0]]] += 1
#                     pass
#
#                 pass
#             # print("==================")
#             pass
#
#             if txt != "":
#                 file_num += 1
#                 f = open(txt_s, mode='wt')
#                 f.write(txt)
#                 f.close()
#                 pass
#             else:
#                 continue
#                 pass
#
#             depthInpaint = depth_raw/65535*255
#             depthInpaint = depthInpaint.astype("uint8")
#
#             transformed = np.dstack([color_raw, depthInpaint])
#
#             # print("transformed shape : " , transformed.shape)
#             # print("row : ",len(transformed), " col : ",len(transformed[0]))
#             print(img_s)
#             cv2.imwrite(img_s, transformed)
#             train_txt += "data/MyDataset_2D/" + str(ele) + ".tiff" + "\n"
#
#
#
#         # print(classes_txt)
#         # splitlinestr = str.splitlines(classes_txt)
#         # print(index)
#         # count_num_str = ""
#         # for i in range(1143):
#         #     # print(splitlinestr[i])
#         #     count_num_str += str(splitlinestr[i]) + "\t"
#         #     count_num_str += str(count_num[i]) + "\n"
#         #
#         # print(count_num_str)
#         # print("classes size : ",len(classes))
#         #
#         # cf = open("/home/user/count.txt", mode='wt')
#         # cf.write(count_num_str)
#         # cf.close()
#
#         # for i in range(18):
#         #     classes_txt += ID[i] + '\n'
#         #     pass
#
#         # cf = open(cls_s, mode='wt')
#         # cf.write(classes_txt)
#         # cf.close()
#         #
#         # tf = open(train_s, mode='wt')
#         # tf.write(train_txt)
#         # tf.close()
#
#         print("file num : ",file_num)


"""=================================================================================================================="""

from scipy import io
from matplotlib import pyplot
from mpl_toolkits.mplot3d import Axes3D
from numpy import matlib
import numpy as np
import cv2
import math
import sys


def flip_towards_viewer(normals, points):
    mat = np.matlib.repmat(np.sqrt(np.sum(points * points, 1)), 3, 1)
    points = points / mat
    # print(points)
    proj = np.sum(points * normals, 1)
    flip = proj > 0
    normals[flip, :] = -normals[flip, :]
    return normals


# np.set_printoptions(threshold=sys.maxsize)

mat_file = io.loadmat('/home/user/SUNRGBD/SUNRGBDMeta.mat')

path_tail = "/home/user/darknet/data/"
# path_tail = "/home/user/"

ID = {"bed": 0, "book": 1, "dishware": 2, "box": 3, "drawer": 4, "chair": 5,
      "counter": 6, "desk": 7, "door": 8, "lamp": 9, "painting": 10, "pillow": 11,
      "plant": 12, "shelf": 13, "sofa": 14, "table": 15, "toilet": 16, "tv": 17}
ID_cnt_train = np.zeros(18)
ID_cnt_valid = np.zeros(18)
# print("ID_cnt :", ID_cnt)
classes = [
    "bed",
    "books",
    "book",
    "bottle",
    "bowl",
    "plate",
    "box",
    "cabinet",
    "drawer",
    "kitchen_cabinet",
    "file_cabinet",
    "chair",
    "kitchen_counter",
    "counter",
    "cup",
    "mug",
    "desk",
    "door",
    "lamp",
    "night_stand",
    "painting",
    "picture",
    "pillow",
    "plant",
    "plants",
    "flowers",
    "shelf",
    "bookshelf",
    "sofa_chair",
    "sofa",
    "table",
    "endtable",
    "coffee_table",
    "dining_table",
    "toilet",
    "tv"
]

max_size = 0
# print("size : ", len(classes))

for range_index in range(4):

    data = mat_file.popitem()
    if (data[0] == 'SUNRGBDMeta'):

        # count_num = np.zeros(1146)
        sample_size = np.arange(10335)
        random_arr = np.random.binomial(n=1, p=0.3, size=10335)
        # sample_size = np.arange(10)

        # print(count_num)
        # classes = {}
        classes_txt = ""
        train_txt = ""
        valid_txt = ""
        cls_s = path_tail + "/pose_yolo.names"
        train_s = path_tail + "/pose_yolo_train.txt"
        valid_s = path_tail + "/pose_yolo_valid.txt"
        index = 0

        pers = 0

        for ele in sample_size:
            # image_index = 0
            # print(ele, "번 째")
            image_index = ele
            if (data[1][0][image_index][1].size < 1):
                continue
                pass
            if image_index % 1000 == 0 and pers < 100:
                pers+=10
                print(pers,"% 진행")

            img_s = path_tail+"pose_yolo/" + str(ele) + ".tiff"
            img_s_valid = path_tail+"pose_yolo_valid/" + str(ele) + ".tiff"
            # img_s_3d = path_tail+"MyDataset_3D/" + str(ele) + ".tiff"

            txt_s = path_tail+"pose_yolo/" + str(ele) + ".txt"
            txt_s_valid = path_tail+"pose_yolo_valid/" + str(ele) + ".txt"

            # train_txt += "data/MyDataset_3D/" + str(ele) + ".tiff" + "\n"
            # img_s_3d = path_tail+"MyDataset/" + str(ele) + ".tiff"

            color_raw = cv2.imread(data[1][0][image_index][5][0], -1)
            depth_raw = cv2.imread(data[1][0][image_index][4][0], -1)
            row = len(color_raw)
            col = len(color_raw[0])

            txt = ""
            for groundtruth3DBB in data[1][0][image_index][1]:
                for items in groundtruth3DBB:

                    if (items[7].size < 1):
                        continue

                    """"check"""
                    label = items[3][0]
                    if label in classes:
                        if label == "books":
                            label = "book"
                            pass
                        elif label == "bottle" or label == "bowl" or label == "plate" or label == "cup" or label == "mug":
                            label = "dishware"
                            pass
                        elif label == "cabinet" or label == "kitchen_cabinet" or label == "file_cabinet" or label == "night_stand":
                            label = "drawer"
                            pass
                        elif label == "kitchen_counter":
                            label = "counter"
                            pass
                        elif label == "picture":
                            label = "painting"
                            pass
                        elif label == "plants" or label == "flowers":
                            label = "plant"
                            pass
                        elif label == "bookshelf":
                            label = "shelf"
                            pass
                        elif label == "sofa_chair":
                            label = "sofa"
                            pass
                        elif label == "endtable" or label == "coffee_table" or label == "dining_table":
                            label = "table"
                            pass
                        pass
                    else:
                        continue
                        pass
                    # ID_cnt[ID[label]] += 1

                    # if items[3][0] not in classes:
                    #     classes[items[3][0]] = index
                    #     index += 1
                    #     classes_txt += items[3][0] + "\n"
                    # print("class : ",items[3][0]," / ",classes[items[3][0]])

                    """ 3D bounding box """

                    # orientation = items[6][0]
                    # # print(orientation)
                    # theta = math.atan2(orientation[1], orientation[0])
                    #
                    # rot_coeffs = np.zeros(3)
                    # if abs(math.cos(theta)) < abs(math.sin(theta)):  # filp
                    #     rot_mat = np.zeros((3, 3))
                    #     rot_mat[0, 0] = math.sin(theta)
                    #     rot_mat[0, 1] = math.cos(theta)
                    #     rot_mat[0, 2] = 0
                    #
                    #     rot_mat[1, 0] = math.cos(theta)
                    #     rot_mat[1, 1] = -math.sin(theta)
                    #     rot_mat[1, 2] = 0
                    #
                    #     rot_mat[2, 0] = 0
                    #     rot_mat[2, 1] = 0
                    #     rot_mat[2, 2] = 1
                    #
                    #     rot_coeffs = items[1][0]
                    #     buf = rot_coeffs[0]
                    #     rot_coeffs[0] = rot_coeffs[1]
                    #     rot_coeffs[1] = buf
                    #     pass
                    # else:  # no flip
                    #     rot_mat = np.zeros((3, 3))
                    #     rot_mat[0, 0] = math.cos(theta)
                    #     rot_mat[0, 1] = -math.sin(theta)
                    #     rot_mat[0, 2] = 0
                    #
                    #     rot_mat[1, 0] = math.sin(theta)
                    #     rot_mat[1, 1] = math.cos(theta)
                    #     rot_mat[1, 2] = 0
                    #
                    #     rot_mat[2, 0] = 0
                    #     rot_mat[2, 1] = 0
                    #     rot_mat[2, 2] = 1
                    #     rot_coeffs = items[1][0]
                    #     pass
                    #
                    #
                    # # if (abs(items[0][0,0]) < abs(items[0][0,1]) and abs(math.cos(theta)) < abs(math.sin(theta)))\
                    # #         or (abs(items[0][0,0]) > abs(items[0][0,1]) and abs(math.cos(theta)) > abs(math.sin(theta)))\
                    # #         or (abs(items[0][0,0]) == abs(items[0][0,1]) and abs(math.cos(theta)) == abs(math.sin(theta))):
                    # #     # print("O")
                    # #     count_O += 1
                    # #     pass
                    # # else:
                    # #     count_X += 1
                    # #     print("basis")
                    # #     print(items[0])
                    # #
                    # #     print()
                    # #     print("rot_mat")
                    # #     print(rot_mat)
                    # #     print("==================================")
                    # #     print("X")
                    #
                    # # print("basis")
                    # # print(items[0])
                    # #
                    # # print()
                    # # print("rot_mat")
                    # # print(rot_mat)
                    # # print("==================================")
                    #
                    # # basis = items[0][inds, :]
                    # # coeffs = items[1][0, inds]
                    #
                    # centroid = items[2]
                    #
                    # t_re = orientation[0]
                    # t_im = orientation[1]
                    #
                    # c1 = rot_coeffs[0]
                    # c2 = rot_coeffs[1]
                    # c3 = rot_coeffs[2]
                    #
                    # x = centroid[0,0]
                    # y = centroid[0,1]
                    # z = centroid[0,2]
                    #
                    # """
                    #     txt = ID[label] t_re t_im c1 c2 c3 x y z
                    #     -1 <= t_re, t_im <= 1
                    #     0 <= c1 c2 c3 <= 1
                    #
                    # """
                    #
                    # txt += str(ID[label])
                    # txt += " " + (str(t_re)+"000000")[:8]
                    # txt += " " + (str(t_im)+"000000")[:8]
                    #
                    # txt += " " + (str(c1)+"000000")[:8]
                    # txt += " " + (str(c2)+"000000")[:8]
                    # txt += " " + (str(c3)+"000000")[:8]
                    #
                    # txt += " " + (str(x)+"000000")[:8]
                    # txt += " " + (str(y)+"000000")[:8]
                    # txt += " " + (str(z)+"000000")[:8] + "\n"

                    """ 2D bounding box """
                    gt2DBB = items[7][0]
                    bx = (items[7][0][0] + items[7][0][2] / 2) / col
                    by = (items[7][0][1] + items[7][0][3] / 2) / row
                    bw = items[7][0][2] / col
                    bh = items[7][0][3] / row
                    orientation = items[6][0]
                    t_re = orientation[0]
                    t_im = orientation[1]

                    # print(label, " : ", items[7][0][0], items[7][0][1], items[7][0][2], items[7][0][3])
                    # print(t_re, ", ", t_im)

                    t_re = (t_re+1)/2
                    t_im = (t_im+1)/2

                    """
                        -1 <= t_re, t_im <= 1
                            ㄴ> 0 ~ 1
                        so, {(t_re, t_im) + 1} / 2
                    """

                    if t_re < 0 or t_re > 1 or t_im < 0 or t_im > 1 :
                        while(1):
                            print("range incorrect")
                        break

                    # print(label, " :: ", bx, by, bw, bh)

                    txt += str(ID[label])
                    txt += " " + (str(bx) + "000000")[:8]
                    txt += " " + (str(by) + "000000")[:8]
                    txt += " " + (str(bw) + "000000")[:8]
                    txt += " " + (str(bh) + "000000")[:8]
                    txt += " " + (str(t_re) + "000000")[:8]
                    txt += " " + (str(t_im) + "000000")[:8] + "\n"

                    if max_size < 1901 and random_arr[image_index] == 1:
                        ID_cnt_valid[ID[label]] += 1
                        pass
                    else:
                        ID_cnt_train[ID[label]] += 1
                        pass

                    # print(label, " theta : ", math.atan2(t_im*2-1, t_re*2-1)*180.0/math.pi)
                    # print("--------------------------------")
                    pass

                pass
            # print("==================")

            # if txt != "":
            #     if max_size < 1901 and random_arr[image_index] == 1:
            #         max_size += 1
            #         f = open(txt_s_valid, mode='wt')
            #         f.write(txt)
            #         f.close()
            #
            #         depthInpaint = depth_raw / 65535 * 255
            #         depthInpaint = depthInpaint.astype("uint8")
            #
            #         transformed = np.dstack([color_raw, depthInpaint])
            #         print(img_s_valid)
            #         cv2.imwrite(img_s_valid, transformed)
            #         valid_txt += "data/pose_yolo_valid/" + str(ele) + ".tiff" + "\n"
            #         pass
            #     else:
            #         f = open(txt_s, mode='wt')
            #         f.write(txt)
            #         f.close()
            #
            #         depthInpaint = depth_raw / 65535 * 255
            #         depthInpaint = depthInpaint.astype("uint8")
            #
            #         transformed = np.dstack([color_raw, depthInpaint])
            #         print(img_s)
            #         cv2.imwrite(img_s, transformed)
            #         train_txt += "data/pose_yolo/" + str(ele) + ".tiff" + "\n"
            #         pass
            #     # print(txt)
            #     # print("==================================")
            #     pass
            # else:
            #     continue
            #     pass

            if max_size == 1901:
                print("last index : ", image_index)
                max_size += 1
                pass

            pass

        # print(classes_txt)
        # splitlinestr = str.splitlines(classes_txt)
        # print(index)
        # count_num_str = ""
        # for i in range(1143):
        #     # print(splitlinestr[i])
        #     count_num_str += str(splitlinestr[i]) + "\t"
        #     count_num_str += str(count_num[i]) + "\n"
        #
        # print(count_num_str)
        # print("classes size : ",len(classes))
        #
        # cf = open("/home/user/count.txt", mode='wt')
        # cf.write(count_num_str)
        # cf.close()

        # cf = open(cls_s, mode='wt')
        # cf.write(classes_txt)
        # cf.close()

        print(ID_cnt_train)
        print(ID_cnt_valid)

        # tf = open(train_s, mode='wt')
        # tf.write(train_txt)
        # tf.close()
        #
        # tf = open(valid_s, mode='wt')
        # tf.write(valid_txt)
        # tf.close()

        # print("end")